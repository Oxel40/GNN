...
class Genome:
    def __init__(self, u_id, dimentions, input_size, output_size, alowed_activation, settings):
        ...
        # tuple of dimentions (layers, space per layer)
        ...
        poss = [random.randrange(0, self.dimentions[0]) + 1,
                random.randrange(0, self.dimentions[1])]
        self.space[poss[0]][poss[1]] = self.Gene(
            self.output_size, (poss[0], poss[1]), {}, random.choice(self.alowed_activation))

        fail = True
        for x in range(self.input_size):
            if random.random() < 0.5:
                self.space[poss[0]][poss[1]].con[self.space[0][x]] = random.uniform(
                    self.settings["new_weight_min"], self.settings["new_weight_max"])
                fail = False
        if fail:
            self.space[poss[0]][poss[1]].con[self.space[0][random.randrange(0, self.input_size)]] = random.uniform(
                self.settings["new_weight_min"], self.settings["new_weight_max"])

        for x in range(self.output_size):
            self.space[-1][x] = self.Gene(x, (self.dimentions[0] + 1, x),
                                          {}, random.choice(self.alowed_activation))
            fail = True
            for o in self.lower_objects(self.space[-1][x].poss[0]):
                if random.random() < 0.5:
                    self.space[-1][x].con[o] = random.uniform(
                        self.settings["new_weight_min"], self.settings["new_weight_max"])
                    fail = False
            if fail:
                self.space[-1][x].con[random.choice(self.lower_objects(self.space[-1][x].poss[0]))] = random.uniform(
                    self.settings["new_weight_min"], self.settings["new_weight_max"])
    ...
    def mutate(self):
        if random.random() < self.settings["node_add_rate"]:
            try:
                ...
                self.space[poss[0]][poss[1]] = self.Gene(self.count_genes(
                ), (poss[0], poss[1]), {}, random.choice(self.alowed_activation))
                self.space[poss[0]][poss[1]].con[random.choice(self.lower_objects(poss[0]))] = random.uniform(
                    self.settings["new_weight_min"], self.settings["new_weight_max"])
                random.choice(self.higher_objects(poss[0])).con[self.space[poss[0]][poss[1]]] = random.uniform(
                    self.settings["new_weight_min"], self.settings["new_weight_max"])
            except:
                pass

        if random.random() < self.settings["node_remove_rate"]:
            nodes = list(set(self.higher_objects(0)).intersection(
                set(self.lower_objects(-1))))
            if nodes != None:
        ...
        if random.random() < self.settings["weight_change_rate"]:
            ...
            for o in gens:
                ...
                for c in pcons:
                    if c in o.con:
                        o.con[c] *= random.uniform(self.settings["weight_change_magnitude_min"],
                                                   self.settings["weight_change_magnitude_max"])
                        done = True
                        break
                if done:
                    break
    ...
    def run(self, feed):
        if len(feed[0]) != self.input_size:
            raise ValueError("Lenght of feed: {0}, does not match the set input_size of: {1}".format(
                len(feed[0]), self.input_size))
        out = []
    ...
    def __init__(self, size, net_dimentions, input_size, output_size, alowed_activation, settings=get_standard_settings()):
        ...
        for o in range(self.size):
            self.individuals.append(
                Genome(o, net_dimentions, input_size, output_size, alowed_activation, settings))
    ...
    def thread(self, thread_id, individuals, feed, results, fitt_func=None, y_=None):
        if fitt_func != None:
            for x in range(len(individuals)):
                results[individuals[x].id] = fitt_func(
                    individuals[x].run(feed), y_)
        else:
            for x in range(len(individuals)):
    ...
    def run(self, feed, fitt_func=None, y_=None):
        ...
        if fitt_func != None:
            for i in range(len(threads)):
                threads[i] = Thread(target=self.thread, args=(
                    i, self.individuals[i::self.settings["computing_threads"]], feed, results, fitt_func, y_))
                threads[i].start()
        else:
            for i in range(len(threads)):
                threads[i] = Thread(target=self.thread, args=(
                    i, self.individuals[i::self.settings["computing_threads"]], feed, results))
                threads[i].start()
        for i in range(len(threads)):
            threads[i].join()
        return results
    ...
    def fitt(self, feed, fitt_func, y_=None, return_best=False):
        ...
        for x in range(1, self.size):
            if x > self.size * self.settings["new_individual_rate"]:
                ...
                self.individuals[x] = Genome(x, self.net_dimentions, self.input_size,
                                             self.output_size, self.alowed_activation, self.settings)
